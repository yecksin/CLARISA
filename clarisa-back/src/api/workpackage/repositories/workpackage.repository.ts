  ...
  async findAllWorkpackages(
    ...
    switch(showWorkpackages){
        ...
        case FindAllOptions.SHOW_ONLY_INACTIVE:
            where = `where stwp.is_active = ${showWorkpackages === FindAllOptions.SHOW_ONLY_ACTIVE ? 1:0} `;
            break;
    }
    ...
    switch(showInitiatives){
        ...
        case FindAllOptions.SHOW_ONLY_ACTIVE:
        case FindAllOptions.SHOW_ONLY_INACTIVE:
            where += `${where?'and': 'where'} stis.is_active = ${showInitiatives === FindAllOptions.SHOW_ONLY_ACTIVE ? 1:0}`;
            break;
    }
    ...
    await Promise.all(workpackages.map(async (w) => {
        let workpackageCountries: WorkpackageCountryDto[] = await this.query(countryQuery, [w.wp_id]);
        let workpackageRegions: WorkpackageRegionDto[] = await this.query(regionQuery, [w.wp_id]);
